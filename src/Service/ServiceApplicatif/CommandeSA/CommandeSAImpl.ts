...
import { FactureDtoFactory } from '../../../Contrainte/Factory/FactureDtoFactory';
...
import { CommandeFactory } from "../../../Contrainte/Factory/CommandeFactory";
...
export class CommandeSAImpl implements CommandeSAAci {
    ...
    async addCommande(commandeDto: any) {
        ...
        let commandeData = this.commandeFactory.createCommandeDo(clientDo, produits, commandeDto);
        if (commandeData.commandeDo.ligneCommande.length !== 0) {
            let commande = await this.commandeSM.addCommande(commandeData.commandeDo);
            await this.produitSM.addOrUpdateProduit(commande.ligneCommande.map(l => l.produit));
